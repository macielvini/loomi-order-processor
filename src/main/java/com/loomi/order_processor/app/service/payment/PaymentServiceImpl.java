package com.loomi.order_processor.app.service.payment;

import com.loomi.order_processor.domain.payment.usecase.ProcessOrderPaymentUseCase;
import org.springframework.stereotype.Service;

import com.loomi.order_processor.domain.order.entity.Order;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Service
public class PaymentServiceImpl implements PaymentService, ProcessOrderPaymentUseCase<String> {

    @Override
    public void processOrderPayment(Order order) {
        this.process(order);
    }

    @Override
    public String process(Order order) {
        log.info("Processing payment for order: {}", order.id());

        try {
            Thread.sleep(100); // Simulate network delay
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }

        log.info("Payment processed successfully for order: {}", order.id());
        return "Success";
    }
}
